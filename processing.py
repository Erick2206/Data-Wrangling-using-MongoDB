...
def process_file(filename, fields):
    ...
    with open(filename, "r") as f:
        ...
        for line in reader:
            ...
            ans=dict(zip(process_fields,temp))
            for item in ans:
                if item=='rdf-schema#label':
                    try:
                        ind=ans[item].index("(")
                        var=ans[item][:ind-1]
                        ans[item]=var
                ...
                if item=='name':
                    if ans[item]=="NULL" or ans[item].isalnum()==False:
                        ans[item]=ans['rdf-schema#label']

                if ans[item]=="NULL":
                    ans[item]=None

                if item=='synonym' and ans[item] is not None:
                    if ans[item][0]=='{':
                        var=ans[item][1:-1]
                        var=var.split("|")
                        for k in var:
